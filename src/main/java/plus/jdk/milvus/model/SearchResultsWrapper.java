...
public class SearchResultsWrapper extends RowRecordWrapper {
    ...
    protected QueryResultsWrapper.RowRecord buildRowRecord(long index) {
        ...
        List<io.milvus.response.SearchResultsWrapper.IDScore> idScore = getIDScore(0);
//        record.put("id", idScore.get((int) index));
     ...
     */
    public List<io.milvus.response.SearchResultsWrapper.IDScore> getIDScore(int indexOfTarget) throws ParamException, IllegalResponseException {
        Position position = getOffsetByIndex(indexOfTarget);
        ...
        List<io.milvus.response.SearchResultsWrapper.IDScore> idScores = new ArrayList<>();
        ...
        if (ids.hasIntId()) {
            ...
            for (int n = 0; n < k; ++n) {
                idScores.add(new io.milvus.response.SearchResultsWrapper.IDScore("", longIDs.getData((int)offset + n), results.getScores((int)offset + n)));
            }
            ...
            for (int n = 0; n < k; ++n) {
                idScores.add(new io.milvus.response.SearchResultsWrapper.IDScore(strIDs.getData((int)offset + n), 0, results.getScores((int)offset + n)));
            }
        } else {
